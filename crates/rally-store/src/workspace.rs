...
impl WorkspaceRepo for Store {
    ...
    fn get(&self, id: WorkspaceId) -> Result<Option<Workspace>, Self::Error> {
        ...
        let mut stmt = conn.prepare_cached(
            "SELECT id, name, repo, created_at, archived FROM workspaces WHERE id = ?1",
        )?;
    ...
    fn list(&self) -> Result<Vec<Workspace>, Self::Error> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare_cached(
            "SELECT id, name, repo, created_at, archived FROM workspaces ORDER BY created_at",
        )?;
    ...
    fn save(&mut self, ws: &Workspace) -> Result<(), Self::Error> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "INSERT INTO workspaces (id, name, repo, created_at, archived)
             VALUES (?1, ?2, ?3, ?4, ?5)
